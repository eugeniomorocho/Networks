...
conn.send("Hello Eugenio, from the EC2 server!".encode()) # Send a message to the connected client. The message must be in bytes, hence the use of the encode() in a UTF-8 encoding method to convert the string into a bytes object. The send() method transmits the data over the established TCP connection. If the client is not ready to receive data, this method may block until the client is ready or until a timeout occurs.
